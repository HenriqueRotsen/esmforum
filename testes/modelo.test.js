...
test('Testando cadastro de três perguntas', () => {
  ...
  const pergunta1 = modelo.get_pergunta(0);
  expect(pergunta1.texto).toBe('1 + 1 = ?')
});

test('Testando resposta das três perguntas', () => {
  ...
  expect(resposta0.texto).toBe('2');
  expect(resposta1.texto).toBe('4');
  expect(resposta2.texto).toBe('6');
});
